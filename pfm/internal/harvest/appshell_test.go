...
func appShellFixture() string {
	...
	b.WriteString(`<style>#no-js-home { display: none; }</style><noscript><style>#no-js-home { display: block; }</style></noscript>`)
	...
	for i := 0; i < 6; i++ {
		b.WriteString(`<p>Example Club is the web application that runs a volunteer community. Organizers and members use it to plan and run meetups, hackdays and conferences across several cities, and keep one shared member and partner directory.</p>`)
	}
...
func shellSite(t *testing.T, jina http.RoundTripper, browser *browserSpyConverter) *Harvester {
	...
	site := roundTripFunc(func(request *http.Request) (*http.Response, error) {
		if request.URL.Host == "defuddle.md" {
			prose := "---\ntitle: Example Club\n---\n" + strings.Repeat("Example Club is the web application that runs a volunteer community. Organizers and members use it to plan and run meetups, hackdays and conferences across several cities. ", 6)
...
func TestAppShellEscalatesToRenderingReader(t *testing.T) {
	...
	if result.Method != "jina" {
		t.Fatalf("app shell accepted at rung %q instead of escalating to jina: rungs=%v content=%.200q", result.Method, result.Rungs, result.Content)
	}
...
func TestAppShellNeverLaunderedThroughNonRenderingReader(t *testing.T) {
	...
	result := h.Fetch(context.Background(), "https://club.example.test/events")
	if result.Error == "" {
		t.Fatalf("app shell returned as SUCCESS (method %q) with no renderer available: %.200q", result.Method, result.Content)
	}
...
func TestAppShellBrowserRenderWins(t *testing.T) {
	rendered := `<html><body><div id="root"><h1>Events</h1>` + strings.Repeat(`<p>Wed 16 Sep — Example Club Amsterdam meetup with 5 hosts and 13 speakers on agent evaluation and retrieval pipelines.</p>`, 6) + `</div></body></html>`
...
func TestServerRenderedPageWithBundleIsNotAShell(t *testing.T) {
	page := `<html><head><script type="module" src="/assets/app.js"></script></head><body><div id="root"><h1>Events</h1>` +
		strings.Repeat(`<p>Wed 16 Sep — Amsterdam meetup, server-rendered with every talk listed in the markup itself.</p>`, 8) + `</div></body></html>`
...
func TestPlainPageIsNeverProbed(t *testing.T) {
	page := `<html><body><article><h1>Notes</h1>` + strings.Repeat(`<p>A plain static article with enough prose to clear the thin-page floor on its own.</p>`, 10) + `</article></body></html>`
	var requests int
	site := roundTripFunc(func(request *http.Request) (*http.Response, error) {
		requests++
		return response(request, http.StatusOK, "text/html", page), nil
...
func TestAppShellWaybackSnapshotOfShellIsRejected(t *testing.T) {
	...
	site := roundTripFunc(func(request *http.Request) (*http.Response, error) {
		if request.URL.Host == "web.archive.org" && strings.Contains(request.URL.Path, appShellProbePrefix) {
			return response(request, http.StatusNotFound, "text/html", "<html><body>Wayback has not archived that URL.</body></html>"), nil
		}
		return response(request, http.StatusOK, "text/html", shell), nil
	})
	archive := roundTripFunc(func(request *http.Request) (*http.Response, error) {
		if request.URL.Host == "archive.org" && request.URL.Path == "/wayback/available" {
			return jsonResponse(request, `{"archived_snapshots":{"closest":{"available":true,"timestamp":"20260901000000"}}}`), nil
		}
		return response(request, http.StatusNotFound, "application/json", `{}`), nil
	})
	missing := roundTripFunc(func(request *http.Request) (*http.Response, error) {
	...
	result := h.Fetch(context.Background(), "https://club.example.test/events")
	if result.Error == "" {
		t.Fatalf("Wayback snapshot of the app shell returned as SUCCESS (method %q, rungs %v)", result.Method, result.Rungs)
	}
	if result.ErrorKind != "app_shell" {
		t.Fatalf("ErrorKind=%q, want app_shell: %q", result.ErrorKind, result.Error)
	}
