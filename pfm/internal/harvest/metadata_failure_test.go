...
func TestMetadataOutageSurvivesMissingLibraryFallback(t *testing.T) {
	...
	for _, mirror := range []string{"", "https://scihub.test"} {
		t.Run("scihub="+mirror, func(t *testing.T) {
			...
			h := mustNew(t, Options{CacheDir: t.TempDir(), OA: oa, Client: client, Chrome: &http.Client{Transport: client.Transport}, SciHubURL: mirror, LibGenURL: "https://libgen.test"})
			for _, mode := range []string{"exact-doi", "oa-pivot"} {
				t.Run(mode, func(t *testing.T) {
					...
					if mode == "exact-doi" {
						...
						got = h.PublicResult(providerFixtureDOI, h.fetchOA(context.Background(), providerFixtureDOI, nil, FetchOptions{}), false)
					}
					if libraryLookups != 1 {
