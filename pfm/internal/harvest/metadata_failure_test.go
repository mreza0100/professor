...
func TestMetadataOutageSurvivesMissingLibraryFallback(t *testing.T) {
	...
	for _, mirror := range []string{"", "https://doi-mirror.test"} {
		t.Run("doi-mirror="+mirror, func(t *testing.T) {
			...
			h := mustNew(t, Options{CacheDir: t.TempDir(), OA: oa, Client: client, Chrome: &http.Client{Transport: client.Transport}, DOIMirrorURL: mirror, MD5CatalogURL: "https://md5-catalog.test"})
