...
func TestMD5CatalogMD5LookupKeepsCookieAndVerifiesPDFDigest(t *testing.T) {
	...
	if got.Error != "" || got.Method != "md5-catalog" || got.Kind != "pdf" || !strings.Contains(got.Content, "md5-catalog converted") {
	...
	bad := h.fetchProviderArtifact(context.Background(), "10.1234/provider.fixture", "md5-catalog", "https://md5-catalog.test/get.php?md5="+wantMD5, "", strings.Repeat("0", 32), FetchOptions{}, []string{"md5-catalog"})
...
func TestIPFSCatalogChallengeIsTerminalAndDoesNotTryGateway(t *testing.T) {
	...
	h := mustNew(t, Options{CacheDir: t.TempDir(), Client: client, Chrome: client, IPFSCatalogURL: "https://ipfs-catalog.test", Converter: &fakeConverter{}})
	got := h.fetchIPFSCatalogMD5(context.Background(), providerFixtureDOI, "9de4a86150a39b54d3e01f98678468bf", FetchOptions{})
...
func TestProviderSearchParsersPreserveBibliographicFields(t *testing.T) {
	...
	candidates, err := providerRecordCandidates([]byte(`<table><tr><td><a href="/edition.php?id=4">Actual book title</a></td><td>Real Author</td><td>Publisher</td><td>2020</td><td><a href="/ads.php?md5=`+md5+`&key=x">GET</a></td></tr></table>`), "https://md5-catalog.test/index.php", "md5-catalog", "caller query", 4)
	if err != nil || len(candidates) != 1 || candidates[0].Title != "Actual book title" || candidates[0].Authors != "Real Author" || candidates[0].Year != 2020 || strings.Contains(candidates[0].Title, "caller query") {
...
func TestProviderSearchCleanMissAndOutageRemainDistinct(t *testing.T) {
	...
	missResolver := &Resolver{Client: missClient, IPFSCatalogURL: "https://ipfs-catalog.test", MD5CatalogURL: "https://md5-catalog.test"}
	...
	outageResolver := &Resolver{Client: &http.Client{Transport: roundTripFunc(func(*http.Request) (*http.Response, error) { return nil, errors.New("provider socket outage") })}, IPFSCatalogURL: "https://ipfs-catalog.test"}
	if _, err := outageResolver.ipfsCatalogSearch(context.Background(), "missing", 4); err == nil || !strings.Contains(err.Error(), "outage") {
