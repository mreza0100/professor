...
- **One implementation each (K3).** Naming precedence, the kill ratchet, row classification, and run-string synthesis each live in exactly ONE package with table-driven tests. A second copy is the bug this rewrite exists to kill. The cross-cutting primitives have one façade each: `atomicfile.Write` for a whole-file replace, `sqlitedb` for every SQLite open, `tmux.Command` for every tmux invocation on a chat socket, and `internal/chat` for every chat verb a surface (CLI, MCP) calls.
- **The architecture ratchet.** `scripts/arch-check.sh` checks C1–C16 against the `.arch/` baselines, and `.claude/scripts/dev.sh verify pfm` runs it. A baseline only shrinks: `--measure` locks a shrink, a new entry is a FAIL to fix, and a genuinely new exception is a hand edit to `.arch/` named in the commit message.
...
Test knobs outside `internal/paths`: the scan clock `PFM_TEST_NOW_NS` (`internal/fleet/scan.go`); `PFM_TEST_FRESH_SOCKET` and `PFM_CODEX_AVAILABLE` (`cmd/pfm/pipeline.go`).
