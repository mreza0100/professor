...
**Frontier seats** — the final judge (step 5), the second-opinion judge (step 4), and the investigate brainer default to the durable `gpt-5.6-sol` alias; a limited-time frontier model rides only the invocation args (`finalJudgeModel`, `securityEscalateModel`, `brainerModel`) per the fleet prompt § Model Selection — never a literal in this file or the script. Security judgment seats never downgrade below `gpt-5.6-sol`.
