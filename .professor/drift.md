...
- Local: `/markdown` was promoted out of this install's drift — the generic half became the machine-global `/quality:forlint` (see `release.md`), and the source project keeps only its own deltas: which live prompt trees route to `/km`, and its private RND evidence paths.
