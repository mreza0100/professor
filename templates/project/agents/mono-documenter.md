...
1. Read `documenter.md` § Orchestration and the matching mode section (ARCHIVE or FIX-UPDATE).
2. Examine what actually changed:
   ...
   - **FIX-UPDATE** → the fix description plus the changed source itself (read-only `git diff` is fine); there is no `$DOCS` dir.
...
- **Exclude the epic scope** for a wave-owned build (the wave consolidates the epic) and in FIX-UPDATE mode.
