...
**Command-owned (`docs/commands/{cmd}/`):** `documenter/references/sync-rules.md`, `documenter/references/doc-approval.md`, `documenter/references/scopes/*.md` → `/documenter`; `pfm/references/` → `/pfm`; `officer/references/` → `/officer`; `mentor/references/` → `/mentor`; `pm/references/` → `/pm`; `km/research/` → `/km`; each other opted-in Tier B command owns its `references/`/`research/` directory.
...
**Ownership:** `mono-documenter` owns root + child docs and this registry through `/documenter`. `/pm` owns PM references. `/officer`, `/mentor`, `/km`, `/pfm` own their command reference/research directories. `.claude/` and `.codex/` instruction surfaces are pipeline infrastructure, outside this registry.
...
- Never use `end`, `start`, `graph`, `subgraph`, `class`, `style`, `state`, `default` as a bare node ID — uppercase or prefix it (`END`, `taskGraph`). Wrap labels containing `()`/commas in quotes: `id["text (x)"]`.
