...
- **/chat:save** — dumps the session's complete working context into a continuation briefing before `/compact` or a fresh chat. The **/chat:** family resumes and coordinates across chats from a pasted excerpt: **/chat:read** (read an earlier chat), **/chat:find** (locate one), **/chat:send** (drop mail into another chat's inbox), **/chat:inject** (force a turn into a live tmux pane or a dormant chat's transcript), **/chat:capture** (snapshot another chat's live window). **/goal-manager** compiles a fuzzy super-goal into a sharp prompt for a fresh session — or `epic {name}` writes an epic's continuation prompt (to consolidate a session into the active epic instead, use `/documenter epic`).
