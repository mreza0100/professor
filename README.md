...
- **/chat:save** — dumps the session's complete working context into a continuation briefing before `/compact` or a fresh chat; **/chat:load** resumes an earlier chat from a pasted excerpt, and **/goal-definer** compiles a fuzzy super-goal into a sharp prompt for a fresh session (to consolidate a session into the active epic instead, use `/documenter epic`).
