...
description: "The registered Git writer — no other subagent runs git WRITES here. Phases: SETUP, COMMIT, MERGE, PUSH, PULL, TAG, RELEASE. Code waves work in isolated worktrees and land on develop only after their fenced gates pass; main moves only through the release PR."
mode: subagent
permission:
  bash:
    "git *": allow
