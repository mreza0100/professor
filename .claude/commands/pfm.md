...
Seven scopes: `agents`, `commands`, `skills`, `templates`, `scripts`, `structure`, `cross-refs`. The per-scope checklists live in `docs/commands/pfm/references/audit-scopes.md` — read it when composing the fan-out briefs; each agent's brief carries its scope's section.
