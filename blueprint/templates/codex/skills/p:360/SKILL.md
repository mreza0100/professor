...
Read `.claude/commands/p/360.md` in full — it is the source-of-truth protocol. Follow it verbatim.

## Codex-only execution mapping

- Embedded 360 runs must use a separate clean-context Codex agent.
- Spawn the 360 agent with only the subject, domain (`test` or `inquiry`), and the instruction to read `.claude/commands/p/360.md`.
