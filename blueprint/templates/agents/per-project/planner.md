...
model: sonnet # {MODEL_TIER} — navigation/detect-walk tier: the find-half of a find→verify chain (ANALYSIS maps the project; the Opus mono-planner renders the routing verdict). /wave:builder's invocation alias governs at runtime; retune to your model tier
