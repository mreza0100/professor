...
| 7   | QA                              | child QA (qa-{project} wrapper)             | Adversarial tests in worktrees + consolidated `$DOCS/6-bugs.md` (one `## {PROJECT}` section each) | worktrees (tests) + root (docs)  |
...
| 9   | Post-merge QA                   | child QA (POST-MERGE, qa-{project} wrapper) | `$DOCS/7-post-merge-qa.md` (single consolidated file from inline results)                         | root                             |
