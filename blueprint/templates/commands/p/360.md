---
name: p:360
description: "360° exhaustive multi-angle analysis. Systematically generates ALL angles on a subject — questions, risks, edge cases, blind spots — organized by dimension. Domains: 'test' (QA), 'inquiry' (Professor/architects), 'auto' (infer from context). Triggered by '360 <subject>', 'three-sixty <subject>', 'do a 360 on <subject>', 'what could go wrong with <subject>', 'find all the angles on <subject>', or referenced by agents at key analysis moments."
---

# 360° — Exhaustive Multi-Angle Analysis

> Where instinct says "think about edge cases," 360° walks every dimension and PROVES each one was considered.
...
## Trace, don't assume — 360° is a fact tracker

Before naming a single angle, READ the subject and everything it touches: open every file, follow every reference and call path, trace each claim back to the code or data behind it, until you know what actually happens — not what probably happens. Every angle must rest on a fact you verified in the source. An assumption is a blind spot in disguise: guess instead of read, and you skip the one angle that mattered. The enumeration is exhaustive; so is the reading beneath it.
...
- Code Auditor runs the **test** domain before category scans (+ **inquiry** for targeted investigations)

Do NOT load for:

- One-shot implementation ("fix X") — that's `/jc` or `/wave:builder`
...
The goal is **exhaustive enumeration, not exhaustive analysis.** Each angle is a one-liner — enough to identify the concern, not to resolve it.
...
| 7   | **Stakeholder conflicts** | {USER_NOUN} vs {SUBJECT_NOUN} vs ops vs legal? Privacy vs UX? Speed vs correctness? Simplicity vs completeness?                                      |
...
### 1. Inputs
- What happens if {entity}_id is a valid UUID but points to a deleted {entity}?
- What if the {payload} is empty string vs null vs missing key entirely?
- ...

### 2. State
- What if this is the {SUBJECT_NOUN}'s very first use (no prior context)?
...
2. **Include in the prompt:** the subject description, the domain (`test` or `inquiry`), and an instruction to read `.claude/commands/p/360.md` and execute the protocol
...
```
Read `.claude/commands/p/360.md` and execute the 360° protocol.
