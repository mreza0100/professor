...
You define the goal; the target session does the work. The deliverable is a **fired `/goal`** — and a goal is ALWAYS inline text, **≤4,000 characters, single-line at fire time, never a path to a file** (deep context rides as repo-file pointers INSIDE the text; the target reads them, the goal itself stays self-standing). The target is any live chat by its tmux name (`/chat:ls`), **including the chat that is calling this command** — self-inject is the only way to fire `/goal` on yourself, since it is a user-typed command (§ Launch). If `$ARGUMENTS` starts with `epic`, run § Subcommand: epic instead of Steps 1–4. The goal arrives spelled out in `$ARGUMENTS` or implied by the conversation: when `$ARGUMENTS` is empty, read what the founder has been driving at in this chat and take that as the super-goal, then refine it in Step 1. Ask for a one-line goal only when the chat gives you nothing to work from. **Non-interactive lane:** a caller that already holds the finished goal (e.g. `/wave:orchestrator` firing the builder's train-wide goal) skips Steps 1–3 and runs § Launch directly with its own ≤4k string — no interrogation, no record-copy detour.
...
```bash
.claude/commands/chat/chat.sh inject --no-sig {target|self} "/goal {the compiled goal string}"
```
