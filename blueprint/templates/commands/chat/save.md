---
name: chat:save
description: Mechanically copy this chat from the registry to tmp/prompt-saves/{name}.md — .claude/commands/chat/chat.sh save dumps the verbatim transcript + git env snapshot straight from disk, no model rewrite. For a model-written briefing on top, use /chat:dump. Trigger — /chat:save {name?}.
argument-hint: [name?]
---

# Chat Save — mechanical transcript copy from the registry

Copy this chat straight from disk — no briefing, no model rewrite. The transcript already lives in the registry; this just dumps it to a file.
