...
> Read `CLAUDE.md` (Professor persona + standards) and `.claude/commands/wave/review.md` § Role: Scout. Enumerate the wave's threads from `{report-path}`. Return the thread manifest only.
...
> Read `CLAUDE.md` and `.claude/commands/wave/review.md` § Role: Walker. Walk this thread end-to-end and return your findings: `{thread spec, verbatim from the manifest}`.

**Phase C — Synthesize (1 agent).** Spawn one agent with the report path and all walker findings:

> Read `CLAUDE.md` and `.claude/commands/wave/review.md` § Role: Synthesizer. Given the report at `{report-path}` and these thread-walk findings `{all walker outputs}`, run the operational review, aggregate the walks, and write the review into the report under `## Professor's Wave Review` per the Report Format. Return the review.
...
4. Run the hygiene lens on the thread's files: read `.claude/commands/audit/code-hygiene.md` and apply its scope-`diff` protocol (duplication & missed reuse first, then hallucinated imports/APIs, over-engineering, dead code, weak types, shallow error handling, naming).
...
- **Scope to the wave's own diff** — a full-codebase sweep is the `audit:code-hygiene` / `audit:security` commands.
