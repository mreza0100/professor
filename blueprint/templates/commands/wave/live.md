...
Write a lightweight review input to `docs/dev/waves/{wave-name}/review.md` — the manifest's task list plus the W5 commit SHAs (the diff the review walks; its Scout runs `git show {sha}` for these JC commits). Check the batch size: ≤3 tasks in the manifest AND the W5 commits' `git diff --name-only` touches no {API_PROTOCOL} schema/resolver file → the walk is a small no-{API_PROTOCOL} batch — today this is intent only: `wave-walker.js` has no size-scoped mode yet, so the walk below always runs its full thread + ledger pass regardless of batch size; a capped-spawn mode for this class is deferred, not wired. Invoke the walker workflow: `Workflow({ scriptPath: '{REPO_ROOT}/.claude/workflows/wave-walker.js', args: { reportPath: 'docs/dev/waves/{wave-name}/review.md' } })` (scriptPath, never `{name}` — name-lookup serves a stale session-start snapshot) — it walks the JC commits' diff two ways in one pass: threads for functional correctness + integration hygiene, and the ledger spine for {API_PROTOCOL} contract/gate mismatches (skipped when the diff has no {API_PROTOCOL} surface), returning `{ verdict, actionItems, review }` plus the `ledger`.
