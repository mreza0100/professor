...
Loaded by `/pfm audit`: PTM reads this file when composing fan-out briefs; each agent's brief carries its scope's section below (adapted per the brief template in `pfm.md § Execution model`). Read-only checks — report `PASS/FAIL/WARN`, never fix.
...
- **Registry coverage:** every command carries `name:` + `description:` frontmatter — the routing signal the harness injects — and the `description:` matches what the command body actually handles and names every subcommand/mode/flag the body defines (`pfm.md § Authoring conventions — Descriptions`); `disable-model-invocation: true` only on user-triggered-by-design commands
...
- **Manifests:** each roster project's manifest is present (`pfm/go.mod`, `engines/wave-walker/engine/package.json`) and its stack description in `CLAUDE.md` matches it
...
- **Agent counts ↔ reality:** a live `ls` of every agents dir → matches `pfm.md § Inventory`'s derivation rules (rosters: `pfm.md § Authoring conventions`, no-rosters law)
...
- **Invariant spot-check:** sample 3 critical invariants from `pfm.md § Critical invariants` → verify they hold in the actual files
- **Co-loaded duplication sweep:** the same rule stated in two co-loaded files (pfm.md ↔ root CLAUDE.md ↔ quality/prompt.md; child CLAUDE.md ↔ root; a command ↔ its reference cards) — each rule lives in exactly ONE canonical home, others carry at most a pointer (quality/prompt anti-patterns 3 & 11)
