...
| pkg.go.dev reachable | **NOT MET** | `curl -o /dev/null -w '%{http_code}'` → 404 for both `pkg.go.dev/github.com/rezzminator/professor` and `.../professor/pfm`. Root cause: `pfm/go.mod` declares `module hostops/pfm` — the module path does not match any GitHub-resolvable import path at all, so pkg.go.dev cannot index it regardless of root placement | Rename the module to `github.com/rezzminator/professor/pfm` (or move pfm to its own repo with a matching module path), push a tag, let pkg.go.dev auto-index (a few minutes after a tagged push) |
