...
| pkg.go.dev reachable | **NOT MET** | `curl -o /dev/null -w '%{http_code}'` → 404 for both `pkg.go.dev/github.com/mreza0100/professor` and `.../professor/pfm`. Root cause: `pfm/go.mod` declares `module hostops/pfm` — the module path does not match any GitHub-resolvable import path at all, so pkg.go.dev cannot index it regardless of root placement | Rename the module to `github.com/rezzminator/professor/pfm` (or move pfm to its own repo with a matching module path), push a tag, let pkg.go.dev auto-index (a few minutes after a tagged push) |
| Go Report Card grade A-/A/A+ | **NOT MET** | `goreportcard.com/report/github.com/mreza0100/professor` returns HTTP 200 but the badge SVG reads **"go report: retired"** — no live grade, consistent with no top-level `go.mod`/importable module | Same fix as above (root-resolvable module) then re-trigger a report at goreportcard.com |
...
**Bottom line:** of the 6 CI-blocking checks, pfm as currently structured fails 3 (go.mod-at-root, pkg.go.dev reachability, Go Report Card grade) — all three trace to the same root cause: `pfm/go.mod`'s module path (`hostops/pfm`) is neither at the repo root nor a GitHub-resolvable import path. The age requirement (5 months) is also short by ~11 days as of 2026-09-14. None of the two closed-PR examples found were rejected for reasons pfm doesn't already share risk on (missing links / multi-item PRs — mechanical, avoidable); pfm's actual blockers are structural, not paperwork.
...
None — all research questions answered from readable sources (`gh api`/`gh pr`/`gh repo view` against `avelino/awesome-go` and `rezzminator/professor`, plus direct `curl` checks of pkg.go.dev and goreportcard.com). One ambiguity noted, not a gap: `gh repo view mreza0100/professor` also resolves to the same repo data as `rezzminator/professor` (same `createdAt`/`pushedAt`/stars) — `gh repo view rezzminator/professor` additionally confirms `nameWithOwner: "rezzminator/professor"` and the git remote `origin` in this checkout points at `https://github.com/rezzminator/professor.git`, so `rezzminator/professor` is treated as canonical throughout this document.
