...
| 5 | **One contract, three runtimes** | `CLAUDE.md` compiles to `AGENTS.md`; `.claude/**` compiles to `.codex/**` (`pfm codex build`) and `.opencode/**` (`build-opencode.mjs`); global agents `.md` → `.toml` twins; the Stop hook recompiles and blocks the turn if a mirror drifts; wave-walker compiled from one source for Claude Workflow and the Codex SDK via `cross-workflow`. | Partly — show the marker line at `AGENTS.md:1` and a `pfm codex check` drift. | `inventory-harness-and-mirrors.md` § B; `inventory-engines-philosophy-releases.md` § wave-walker |
...
| 8 | **Wave pipeline + wave-walker** | refine → scheduler → orchestrator → builder → walker; the walker is a 19-seat verification engine (scout, thread walkers, slice sensor, gate sweep, security auditor, invariant hunter, anomaly judge, coverage critic, second opinion, final judge) whose "SCOUT FAILURE" is a named state; invariants registry the walk reads. | Partly — a walk takes minutes; show a finished review. | `inventory-engines-philosophy-releases.md` § wave-walker; discipline-layer § wave commands |
...
| 13 | **Optional cast + legal shelf** | `/officer`, `/km`, `/pm`, `/mentor`, `/marketer`; 11 legal playbooks (DPA, DPIA, breach, notices, vendor DD, red-team). | No. | `inventory-discipline-layer.md` |
...
- `## Engines` names `engines/rr/` — does not exist; the engine is `engines/deep-rr/`.
- `engines/wave-walker/` has no README; the truth is `engine/design.md`.
